...
describe('AuthService - OAuth', () => {
  ...
  beforeEach(() => {
    ...
    oauthServerMock.waitForCode.mockResolvedValue({ code: 'test_code', state: 'test_state' });
  ...
  it('completes OAuth flow successfully', async () => {
    ...
    const mockUser = { id: 123, login: 'oauthuser', avatar_url: 'https://example.com/avatar.png' };
    const mockOctokit = {
      rest: {
        users: {
          getAuthenticated: vi.fn().mockResolvedValue({ data: mockUser })
...
describe('AuthService - exchangeCodeForToken', () => {
  ...
  function callExchange(code: string, clientId: string, clientSecret: string): Promise<string> {
    const target = authService as unknown as { exchangeCodeForToken: (c: string, ci: string, cs: string) => Promise<string> };
    return target.exchangeCodeForToken.call(authService, code, clientId, clientSecret);
  }
  ...
  it('throws on non-ok response', async () => {
    ...
    await expect(callExchange('code', 'client_id', 'client_secret')).rejects.toThrow('Token exchange failed: 403 Forbidden');
  });
  ...
  it('throws when response has no access_token', async () => {
    ...
    await expect(callExchange('code', 'client_id', 'client_secret')).rejects.toThrow('no access_token in response');
  });
