...
vi.mock('../../github/github-service', () => ({
  githubService: { getOctokit: () => mocks.octokit }
}));
...
describe('AgentGitBridge', () => {
  ...
  describe('createPR', () => {
    it('creates a PR from worktree', async () => {
      ...
      mocks.octokit.rest.pulls.create.mockResolvedValue({
        data: { number: 234, html_url: 'https://github.com/test-owner/test-repo/pull/234' }
      });

      const result = await bridge.createPR('/path/to/worktree', 'Fix auth', 'Resolves #123');
      
      expect(result.number).toBe(234);
      expect(result.url).toBe('https://github.com/test-owner/test-repo/pull/234');
    ...
    it('handles SSH remote URLs', async () => {
      ...
      const result = await bridge.createPR('/worktree', 'Update docs', 'Added docs');
  ...
  describe('readPRFeedback', () => {
    ...
    it('handles comments with missing user', async () => {
      ...
      expect(result).toEqual([
        { user: 'unknown', body: 'Anonymous comment' }
      ]);
    });
    ...
    it('propagates API errors', async () => {
      ...
      await expect(
        bridge.readPRFeedback('owner', 'repo', 1)
