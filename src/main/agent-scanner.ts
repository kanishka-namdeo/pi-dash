...
import type { AgentConfig, KnownAgent, ScanResult, ValidationResult, IdentificationResult } from '../shared/types';
...
export async function identifyAgent(agentPath: string): Promise<IdentificationResult> {
...
async function checkLocation(dir: string, binary: string): Promise<string | null> {
  try {
    await fs.access(dir);
