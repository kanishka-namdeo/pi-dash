...
- `data/` — Mock/seed data for simulation (single file: `mockData.ts` with `seedAgents`, `seedActivities`, `seedPlanSteps`, `activityTemplates`)
- `hooks/` — All stateful logic
  ...
  - `useActivityFeed` — Live activity feed generation from active agents
  - `useAgentSimulation` — Agent progress simulation with pause/resume/reset
