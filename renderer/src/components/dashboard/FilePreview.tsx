		...
		const fetchContent = async () => {
			...
			try {
				const result: FileContentResult =
					await window.api.filetree.getFileContent(path);
				...
				{fileSize > 1024 * 1024 && (
					...
					>
						...
						<div>
							...
							<p className="text-sm" style={{ color: "var(--text-muted)" }}>
								This file ({formatFileSize(fileSize)}) exceeds the 1MB preview
								limit.
							</p>
						</div>
						<div className="flex items-center justify-center gap-2 pt-2">
				...
				{content && !isLoading && !error && (
					...
					>
						...
						>
							<div className="flex items-center gap-2">
								<button
									...
									style={{
										backgroundColor: isMarkdownMode
											? "var(--accent-indigo)"
											: "var(--bg)",
						...
						>
							{path
								.toLowerCase()
								.match(/\.(png|jpe?g|gif|bmp|webp|ico|svg)$/i) ? (
								<img
